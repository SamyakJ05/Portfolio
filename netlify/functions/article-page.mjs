...
<head>
  ...
  <style>
    ...
    .read-nav-left { display: flex; align-items: center; gap: 16px; min-width: 0; }
    .read-nav-logo {
    ...
    .read-nav-logo:hover { color: var(--accent); }
    .read-nav-sep { color: var(--border); flex-shrink: 0; }
    .read-nav-title {
    ...
    .read-nav-title.visible { opacity: 1; transform: translateY(0); }
    .read-nav-right { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
    .read-nav-back {
    ...
    .art-toc-list { list-style: none; display: flex; flex-direction: column; gap: 8px; }
    .art-toc-item a {
    ...
    .art-body h2 {
      ...
      margin: 60px 0 18px; color: var(--text);
    ...
    .art-body ul, .art-body ol { padding-left: 26px; margin-bottom: 26px; }
    .art-body li { margin-bottom: 8px; line-height: 1.75; }
    .art-body a { color: var(--accent); text-decoration: underline; text-underline-offset: 3px; transition: opacity 0.2s; }
    .art-body a:hover { opacity: 0.75; }
    .art-body strong { color: var(--text); font-weight: 600; }
    .art-body em { color: var(--muted); }
    .art-body img { max-width: 100%; height: auto; border-radius: 12px; display: block; margin: 32px 0; }
    .art-cover {
      width: 100%; aspect-ratio: 40 / 21; object-fit: cover;
      border-radius: 16px; border: 1px solid var(--border);
      display: block; margin: 8px 0 44px;
    }
    .art-body blockquote {
      border-left: 3px solid var(--accent); padding: 2px 0 2px 24px;
      margin: 36px 0; color: var(--muted); font-style: italic; font-size: 1.05em;
    }
    .art-body code {
    ...
    .art-body pre code {
      display: block; background: none; border: none; padding: 20px 24px;
    ...
    .art-share-label { font-size: 0.78rem; color: var(--muted); margin-right: 4px; flex-shrink: 0; }
    .share-btn {
      display: inline-flex; align-items: center; gap: 6px;
      font-size: 0.78rem; padding: 7px 14px; border-radius: 8px;
    ...
    .art-author {
      ...
      border-radius: 14px; margin-top: 48px;
    }
    .art-author-avatar {
...
<nav class="read-nav" id="read-nav">
  ...
  <div class="read-nav-right">
    ...
    <button id="theme-toggle" class="nav-toggle" aria-label="Toggle theme"></button>
  </div>
</nav>
...
<main>
  <div class="art-wrap" id="article-page">
    ...
    <div class="art-author">
      <div class="art-author-avatar">SJ</div>
      <div>
        <div class="art-author-name">Samyak Jain</div>
...
<script>
(function() {
  // Old links (/article?slug=..., /article.html?slug=...) 301 in here, but
  // Netlify's edge appends the original request's query string to every
  // redirect Location regardless of the target, so visitors can land on
  // this canonical path with a stray ?slug=... still attached. The
  // canonical tag already tells crawlers the clean URL is authoritative;
  // this just tidies the address bar for human visitors.
  if (location.search) {
